...
def get_stock_data(code, name):
    ...
    try:
        ...
        if rs.error_code == '0':
            ...
            if len(df) >= 100:
                bs.logout()
                return process_bs_data(df), 'baostock'
    ...
    # baostock失败，使用高质量模拟数据
    return generate_sim_data(code, name), 'simulation'


def process_bs_data(df):
    """处理baostock数据"""
...
def generate_sim_data(code, name, days=8):
    """生成高质量模拟数据"""
    ...
    base_prices = {
        ...
        '688256': 135, '300476': 8.5, '000001': 10.5,
...
def main():
    ...
    for idx, (code, name, sector) in enumerate(STOCK_POOL):
        ...
        df, source = get_stock_data(code, name)
    ...
    print(f"   2. 真实市场受政策、消息、资金情绪、板块轮动等多重因素影响")
    print(f"   3. 预测准确率不可能100%，历史表现不代表未来")
    print(f"   4. 请务必独立判断，切勿盲目跟单")
    print(f"   5. 投资有风险，入市需谨慎，请对自己的投资负责")
